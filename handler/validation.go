...
func (validation *ValidationRegister) validateFullName(params generated.RegisterRequest) error {
	...
	if len(params.FullName) < 3 {
		validation.PhoneNumberValidation = "Full name must be at least 3 characters"
		err = errors.New("fail validation")
	} else if len(params.FullName) > 60 {
		validation.PhoneNumberValidation = "Full name must be less than 60 characters"
		err = errors.New("fail validation")
	}
	return err
}
...
func (validation *ValidationRegister) validatePassword(params generated.RegisterRequest) error {
	...
	if len(params.Password) < 6 {
		validation.PhoneNumberValidation = "Password must be at least 6 characters"
		err = errors.New("fail validation")
	} else if len(params.Password) > 64 {
		validation.PhoneNumberValidation = "Password must be less than 64 characters"
		err = errors.New("fail validation")
	} else {
		...
		if !passwordValidaton {
			err = errors.New("fail validation")
			validation.PhoneNumberValidation = "Password must contain at least 1 capital characters"
		}
		regex, _ = regexp.Compile(`\d`)
		passwordValidaton = regex.MatchString(params.Password)
		if !passwordValidaton {
			err = errors.New("fail validation")
			validation.PhoneNumberValidation = "Password must contain at least 1 number"
		}
		regex, _ = regexp.Compile(`[\W_]`)
		passwordValidaton = regex.MatchString(params.Password)
		if !passwordValidaton {
			err = errors.New("fail validation")
			validation.PhoneNumberValidation = "Password must contain at least 1 special (non alpha-numeric) characters"
		}
	}
